...
async function setupSocketIO(httpServer: any) {
  ...
  const progressChannels = ['thumbnailGenerate:progress', 'thumbnailEdit:progress'];
    ...
    progressSub.on('message', (channel, message) => {
      if (progressChannels.includes(channel)) {
        try {
          ...
          console.log(`[${channel}] Broadcasting progress for ${data.thumbnailVersionId}: ${data.status} ${data.pct}%`);
          
          // Determine operation type from channel
          const operation = channel === 'thumbnailGenerate:progress' ? 'generate' : 'edit';
          ...
          io.to(data.thumbnailVersionId).emit('thumbnail:progress', enrichedData);
          io.to(data.thumbnailVersionId).emit(`thumbnail${operation}:progress`, enrichedData);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-thumbnail', (thumbnailVersionId) => {
      console.log(`Client ${socket.id} joining thumbnail room: ${thumbnailVersionId}`);
      if (!thumbnailVersionId) return;
      
      socket.join(thumbnailVersionId);
      console.log(`Client ${socket.id} joined thumbnail room: ${thumbnailVersionId}`);
    ...
    socket.on('join-thumbnail-generate', (thumbnailVersionId) => {
      console.log(`Client ${socket.id} joining thumbnail generate room: ${thumbnailVersionId}`);
      if (!thumbnailVersionId) return;
      
      socket.join(`${thumbnailVersionId}:generate`);
      socket.emit('joined-generate', { thumbnailVersionId, room: `${thumbnailVersionId}:generate` });
    });
    
    socket.on('join-thumbnail-edit', (thumbnailVersionId) => {
      console.log(`Client ${socket.id} joining thumbnail edit room: ${thumbnailVersionId}`);
      if (!thumbnailVersionId) return;
      
      socket.join(`${thumbnailVersionId}:edit`);
      socket.emit('joined-edit', { thumbnailVersionId, room: `${thumbnailVersionId}:edit` });
    });
    ...
    socket.on('leave-thumbnail', (thumbnailVersionId) => {
      ...
      console.log(`Client ${socket.id} left thumbnail rooms: ${thumbnailVersionId}`);
    });
